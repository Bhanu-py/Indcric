...
from .models import BotEvent, Session, User, Vote
...
# ──────────────────────────────────────────────
# Meta Cloud API webhook  (/api/bot/whatsapp/)
# ──────────────────────────────────────────────
...
def _handle_whatsapp_message(request):
    ...
    # Meta expects 200 OK quickly; process and return
    try:
...
def _process_message(msg, value):
    ...
    # Determine intent: yes / no / balance (extensible later)
    if text in ('yes', 'y', '✅', '1'):
...
def _handle_rsvp(wa_message_id, phone, choice, raw):
    ...
    # Find the most recent open poll for this user's club
    poll = (
        Session.objects
...
# ──────────────────────────────────────────────
# Internal RSVP webhook  (/api/bot/rsvp/)
# Token-authed, for programmatic use
# ──────────────────────────────────────────────
