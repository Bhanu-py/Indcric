...
def send_text_message(to_phone, text):
    """Send a free-text WhatsApp message (only within 24h conversation window)."""
    if not settings.WHATSAPP_PHONE_NUMBER_ID or not settings.WHATSAPP_ACCESS_TOKEN:
        logger.warning("WhatsApp credentials not configured — skipping DM to %s", to_phone)
        return False
...
def notify_poll_created(poll):
    ...
    Uses the 'session_rsvp' template — must be approved in WhatsApp Manager first.
    ...
    logger.info("Poll created for session %s — notified %d/%d members", session.name, sent, users_with_phone.count())
    return sent
